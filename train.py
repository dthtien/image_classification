...
from yolo3.model_v2 import create_model
from yolo3.utils_v3 import update_path, get_anchors, get_dataset_class_names, get_nb_classes, data_generator
...
class Trainer:
  def __init__(self, train_set, classes_path):
    self.path_dataset = train_set
    self.classes_path = classes_path

  def _export_classes(self, class_names, path_output):
  ...
  def _export_model(self, model, path_output, name_prefix, name_sufix):
  ...
  def load_training_lines(self, path_annot, valid_split):
  ...
  def execute(self):
    ...
    path_weights = 'model_data/pretraining.h5'
    path_output = 'training_output'
    path_classes = self.classes_path
    nb_gpu = 0
    ...
    self._export_classes(get_dataset_class_names(path_dataset, path_classes), path_output)

    is_tiny_version = len(anchors) == 6  # default setting
    name_prefix = 'tiny-' if is_tiny_version else ''
    model = create_model(config['image-size'], anchors, nb_classes, freeze_body=2,
    ...
    lines_train, lines_valid, num_val, num_train = self.load_training_lines(path_dataset,
                                                                        config['valid-split'])
    ...
    if config['epochs'].get('head', 0) > 0:
      ...
      self._export_model(model, path_output, name_prefix, '_head')
    ...
    model.fit_generator(_data_generator(lines_train, batch_size=config['batch-size']['full']),
                        ...
                        callbacks=[tb_logging, checkpoint, reduce_lr, early_stopping])
    logging.info('Training took %f minutes', (time.time() - t_start) / 60.)
    self._export_model(model, path_output, name_prefix, '_final')

trainer = Trainer(train_set = 'train.txt', classes_path = 'train_classes.txt')
trainer.execute()
